...
<script>
    ...
    gsap.from(".content-body h3", {
        opacity: 0,
        y: 30,  // Appears from below
        duration: 1,
        ease: "power3.out"
    });

    // Animate the "Meeting Schedule" and "Date" from the bottom
    ...
    // Animate the search input box from the left
    ...
    // Animate each table row appearing one by one with a delay
    const rows = document.querySelectorAll("tbody tr");
    rows.forEach((row, index) => {
        const cells = Array.from(row.querySelectorAll('td')).slice(0, -1);  // Exclude the last td (action buttons)
