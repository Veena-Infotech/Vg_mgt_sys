...
<body>
    ...
    <script>
        ...
        noUiSlider.create(rangeSlider, {
            start: [<?= $start_min ?>, <?= $start_max ?>],  // Set dynamic start range
            connect: true,
            range: {
                'min': <?= $min_value ?>,  // Min value fetched from DB
                'max': <?= $max_value ?>   // Max value fetched from DB
        },
            format: {
                to: function (value) {
                    return Math.round(value); // Display rounded value
                },
                from: function (value) {
                    return Number(value);
                }
            },
            tooltips: [true, true]  // Show tooltip for both handles
        });

        // Listen for changes and update the hidden input field
        rangeSlider.noUiSlider.on('update', function (values, handle) {
            document.getElementById('range-value').value = values.join(',');
        });
    </script>
